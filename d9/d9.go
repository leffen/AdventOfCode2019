...
// Opcode 3
func (p *program) store(op *opCode, inp int) {
...
// opcode 9
func (p *program) adjustRelativeBase(op *opCode) int {
	v1 := p.valByMode(p.curr+1, 1)
	p.realtiveBase += v1
	p.curr += 2
	return p.realtiveBase
}

func (p *program) getValueAt(pos int) int {
	if pos < 0 {
		logrus.Fatalf("Pos %d is below zero", pos)

	}
	if pos > len(p.items) {
		logrus.Fatalf("Pos %d is bigger than length %d", pos, len(p.items))
	}
...
func (p *program) valByMode(pos, mode int) int {
	...
	// Relative base mode
	if mode == 2 {
		return p.realtiveBase + p.items[pos]
	}

	return p.getValueAt(p.items[pos])
}

func (p *program) posByMode(pos, mode int) int {
	if mode == 0 {
		return p.getValueAt(pos)
	}
	if mode == 2 {
		return p.realtiveBase + p.items[pos]
	}
