...
func (p *program) walk(nr, dist int, dx, dy int, mrk string) {
	for i := 0; i < dist; i++ {
		...
		p.posY += dy
		p.step++
		p.grd.setXy(p.posX, p.posY, nr, mrk,p.step)
	}
}
...
func (p *program) leastSteps() int {
	...
	for _, c := range p.grd.cells {
		if c.cnt > 1 {
			steps := 0
			for _,v := range c.xstep{
				steps += v
			}
			fmt.Printf("X Cell %v steps=%d\n", c, steps)
			if steps < fewestSteps {
				fmt.Printf("Least steps to %d from %d\n", steps, fewestSteps)
				fewestSteps = steps
			}
		}
